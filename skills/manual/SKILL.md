...
description: >
  ...
  Use this skill whenever the user asks about Manual, the `manual` CLI, workflows,
...
```
CLI (`manual`) ──JSON-RPC──▶ App Server ──▶ 에이전트 실행기 (Claude / Codex / Pi)
...
- **CLI** (`app/cli/src/main.rs`) — `manual` 바이너리
...
manual --server-bin ./manual-rs/target/debug/manual-app-server workflow list

# 3. 이미 실행 중인 서버에 HTTP 연결
manual --server-url http://127.0.0.1:PORT --auth-token TOKEN workflow list
```

환경변수: `MANUAL_APP_SERVER_BIN`, `MANUAL_APP_SERVER_URL`, `MANUAL_APP_SERVER_TOKEN`, `MANUAL_APP_SERVER_DISCOVERY`

기본 discovery 파일: `~/.manual/app-server.json`
...
manual workflow update <workflow_id> updated.json  # 전체 교체
manual workflow delete <workflow_id>     # 삭제
```
...
manual workflow start <workflow_id>

# 실행 시작 + 완료까지 이벤트 스트림 폴링
manual workflow run <workflow_id>

# 단계별 수동 진행 (각 노드 완료 후 paused; resume으로 진행)
manual workflow start <workflow_id> --mode step

# 특정 노드부터 실행
manual workflow start <workflow_id> --start-node <node_id>

# 이전 실행의 첫 실패 노드부터 재시작
manual workflow start <workflow_id> --resume-from-failure --resume-run-id <previous_run_id>

# 노드 입력 오버라이드
manual workflow start <workflow_id> --inputs overrides.json

# 이벤트 조회
manual workflow events <run_id>                  # 한 번 조회
manual workflow events <run_id> --watch          # 완료까지 폴링
manual workflow events <run_id> --cursor 5       # 5번부터 조회

# 실행 중단 / 재개
manual workflow stop <run_id>
...
manual rpc workflow.patch params.json   # 부분 수정 (CLI 미래핑)
echo '{"workflow_id":"x"}' | manual rpc workflow.get -
```
