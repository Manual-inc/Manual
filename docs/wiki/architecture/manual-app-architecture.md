...
- Windows 앱은 아직 app-server live wiring 전이지만, 최소한 `doctor` -> `demo` -> `starter` -> `output` quick-start narrative와 workflow run summary, optimization metric cards를 먼저 갖춘다.
